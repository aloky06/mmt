	...
	class MenuWalker
	{
		...
		public function generateTree($items = [],$depth = 0,$parentKey = '')
		{
			...
			foreach ($items as $k=>$item) {
				...
				$iconHtml = '';
				if (!empty($item['icon_image_url'])) {
					$iconHtml = sprintf('<img src="%s" class="custom-menu-icon" alt="icon">', e($item['icon_image_url']));
				} elseif (!empty($item['icon_class'])) {
					$iconHtml = sprintf('<i class="%s menu-icon"></i>', e($item['icon_class']));
				}
				
				printf('<a target="%s" href="%s">%s <span class="menu-text">%s</span></a>', e($item['target']), e($url), $iconHtml, clean($item['name']));
				if (!empty($item['children'])) {
