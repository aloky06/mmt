...
//                    $query->update(['status' => $action]);
    ...
    /**
     * Pending Hotel Verification Queue
     */
    public function pendingVerification(Request $request)
    {
        $this->checkPermission('hotel_view');
        $query = $this->hotelClass::where('status', 'pending')->with(['author'])->orderBy('id', 'desc');
        $data = [
            'rows'        => $query->paginate(20),
            'breadcrumbs' => [
                ['name' => __('Hotels'), 'url' => 'admin/module/hotel'],
                ['name' => __('Pending Verification'), 'class' => 'active'],
            ],
            'page_title'  => __('Pending Hotel Verification'),
        ];
        return view('Hotel::admin.hotel.pending', $data);
    }
    ...
    public function approveHotel(Request $request, $id)
    {
        ...
        event(new \Modules\Core\Events\UpdatedServiceEvent($row));

        return redirect()->back()->with('success', __('Hotel approved and published successfully!'));
    }
    ...
    public function rejectHotel(Request $request, $id)
    {
        ...
        $policy['rejection_reason'] = $rejectionReason;
        $policy['rejected_at'] = now()->toDateTimeString();
        $row->policy = $policy;
        ...
        return redirect()->back()->with('success', __('Hotel rejected and owner notified.'));
    }
}
