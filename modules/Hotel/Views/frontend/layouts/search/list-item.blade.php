    ...
    <div class="container mx-auto px-4" style="max-width:1200px;">
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="w-full lg:w-3/4 flex flex-col">
                ...
                {{-- Sort Bar --}}
                @include('Hotel::frontend.layouts.search.sort-bar')

                {{-- Hotel Cards --}}
                <div class="flex flex-col">
                    @if($rows->total() > 0)
                        @foreach($rows as $row)
                            @include('Hotel::frontend.layouts.search.hotel-card')
                        @endforeach
                    @else
                        <div class="bg-white rounded-xl border border-gray-200 p-12 text-center shadow-sm">
                            ...
                            <h4 class="text-xl font-bold text-gray-900 mb-2">No Properties Found</h4>
                            <p class="text-gray-500">We couldn't find any properties matching your current filters. Try adjusting them or searching a different area.</p>
                            <button onclick="window.location.href='{{ route('hotel.search') }}'" class="mt-6 bg-brand hover:bg-brand-dark text-white font-bold py-2.5 px-6 rounded-lg transition-colors">
                                Clear All Filters
                            </button>
                        </div>
                    @endif
                ...
                @if($rows->total() > 0)
                    <div class="mt-8 flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                        <div class="text-sm font-medium text-gray-500">
                            Showing <span class="font-bold text-gray-900">{{ $rows->firstItem() }}</span> to <span class="font-bold text-gray-900">{{ $rows->lastItem() }}</span> of <span class="font-bold text-gray-900">{{ $rows->total() }}</span> properties
                        </div>
