<div class="form-group custom-location-group" id="ht-location-wrapper" onclick="document.getElementById('ht-location-input').focus()">
    ...
    <div class="form-content" style="position:relative;">
        ...
        <input type="text" id="ht-location-input" placeholder="{{__("Where are you going?")}}" autocomplete="off" onkeyup="searchHotelLocation(this.value)" onclick="searchHotelLocation(this.value)" value="{{ $location_name }}">
        
        <!-- Hidden Inputs for Form Submission -->
        <input type="hidden" name="location_id" id="ht-location_id" value="{{request()->input('location_id')}}">
        <input type="hidden" name="map_place" id="ht_map_place" value="{{request()->input('map_place')}}">
        <input type="hidden" name="map_lat" id="ht_map_lat" value="{{request()->input('map_lat')}}">
        <input type="hidden" name="map_lgn" id="ht_map_lgn" value="{{request()->input('map_lgn')}}">

        <!-- Dropdown Container -->
        <div class="premium-autocomplete-dropdown" id="ht-location-dropdown"></div>
    </div>
</div>
...
<script>
    ...
    const searchHotelLocation = debounce(async function(query) {
        const dropdown = document.getElementById('ht-location-dropdown');
        dropdown.innerHTML = '';
            ...
            if (data.features && data.features.length > 0) {
                data.features.forEach(feature => {
                    ...
                    item.onclick = function(e) {
                        ...
                        selectHtLocation(name, geom[1], geom[0], subText);
                    };
    ...
    function selectHtLocation(name, lat, lng, subtitle) {
        document.getElementById('ht-location-input').value = name;
        document.getElementById('ht_map_place').value = name;
        document.getElementById('ht_map_lat').value = lat;
        document.getElementById('ht_map_lgn').value = lng;
        document.getElementById('ht-location-dropdown').style.display = 'none';
    }

    // Hide dropdowns when clicking outside
    document.addEventListener('click', function(e) {
        let wrapper = document.getElementById('ht-location-wrapper');
        let dropdown = document.getElementById('ht-location-dropdown');
