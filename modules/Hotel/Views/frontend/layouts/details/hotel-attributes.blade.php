...
@if(!empty($terms_ids) and !empty($attributes))
    <div class="g-all-attribute bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border-0 p-6 md:p-8 mb-8">
        @foreach($attributes as $key => $attribute )
            ...
            @if(empty($attribute['parent']['hide_in_single']))
                <div class="g-attributes {{$attribute['parent']->slug}} attr-{{$attribute['parent']->id}} mb-6 last:mb-0">
                    <h3 class="text-xl font-black text-gray-900 mb-4 tracking-tight">{{ $translate_attribute->name }}</h3>
