...
class VendorController extends FrontendController
{
    ...
    public function delete($id)
    {
        ...
        if(\request()->query('permanently_delete')) {
            $query = $this->hotelClass::where("create_user", $user_id)->where("id", $id)->withTrashed()->first();
            if (!empty($query)) {
                $query->forceDelete();
            }
        }else {
    ...
    public function dashboard(Request $request, $id)
    {
        ...
        $user_id = Auth::id();
        $row = $this->hotelClass::where('create_user', $user_id)->find($id);
        ...
        // Get booking stats
        $totalBookings   = $this->bookingClass::where('object_id', $id)->where('object_model', 'hotel')->count();
        $confirmedCount  = $this->bookingClass::where('object_id', $id)->where('object_model', 'hotel')->where('status', 'completed')->count();
        $pendingCount    = $this->bookingClass::where('object_id', $id)->where('object_model', 'hotel')->where('status', 'pending')->count();
        $totalRevenue    = $this->bookingClass::where('object_id', $id)->where('object_model', 'hotel')->where('status', 'completed')->sum('total');
        $recentBookings  = $this->bookingClass::where('object_id', $id)->where('object_model', 'hotel')->orderBy('id', 'desc')->limit(10)->get();
        ...
        $data = [
            ...
            'breadcrumbs'    => [
                ['name' => __('Manage Hotels'), 'url' => route('hotel.vendor.index')],
                ['name' => $row->title, 'url' => route('hotel.vendor.edit', ['id' => $row->id])],
                ['name' => __('Dashboard'), 'class' => 'active'],
            ],
            'page_title' => __('Hotel Dashboard') . ': ' . $row->title,
        ];

        return view('Hotel::frontend.vendorHotel.dashboard', $data);
    }
}
