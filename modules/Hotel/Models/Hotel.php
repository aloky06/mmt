    ...
    public static function search(Request $request)
    {
        ...
        if (!empty($location_id = $request->query('location_id'))) {
            ...
            if ($country_location_ids !== false) {
                $model_hotel->whereIn("bravo_hotels.location_id", $country_location_ids);
            }
        }
        ...
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
            $distance = 50; // 50 km radius limit
            $model_hotel->whereRaw("( 6371 * acos( cos( radians(?) ) *
                            cos( radians( map_lat ) )
                            * cos( radians( map_lng ) - radians(?)
                            ) + sin( radians(?) ) *
                            sin( radians( map_lat ) ) )
                            ) <= ?", [$lat, $lgn, $lat, $distance]);
            $model_hotel->orderByRaw("( 6371 * acos( cos( radians(?) ) *
                            cos( radians( map_lat ) )
                            * cos( radians( map_lng ) - radians(?)
                            ) + sin( radians(?) ) *
                            sin( radians( map_lat ) ) )
        ...
        switch ($orderby){
            ...
            case "rate_high_low":
                ...
                break;
            default:
                // Default sorting: Lowest to Highest Price
                $raw_sql = "CASE WHEN IFNULL( bravo_hotels.sale_price, 0 ) > 0 THEN bravo_hotels.sale_price ELSE bravo_hotels.price END AS tmp_min_price";
                $model_hotel->selectRaw($raw_sql);
                $model_hotel->orderBy("tmp_min_price", "asc");
        }
