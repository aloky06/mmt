    ...
    <div class="owl-carousel">
        @if(!empty($list_slider))
            @foreach($list_slider as $item)
                ...
                <div class="item" style="background-image: linear-gradient(0deg,rgba(0, 0, 0, 0.2),rgba(0, 0, 0, 0.2)),url('{{$img_url}}') !important">
                    <div class="item-title text-center">
                        <div class="container">
                            <h1 class="text-heading">{{$item['title'] ?? ""}}</h1>
                            <div class="sub-heading">{{$item['desc'] ?? ""}}</div>
                        </div>
                    </div>
                </div>
            @endforeach
