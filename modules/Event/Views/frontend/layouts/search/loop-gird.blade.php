@php
    ...
    // Event Specifics
    $duration = duration_format($row->duration);
    $start_time = $row->start_time;
    
    // Review Data
    $reviewData = setting_item('event_enable_review') ? $row->getScoreReview() : null;
@endphp
            ...
            <div>
                ...
                <div class="flex items-center gap-2 mb-3">
                    ...
                    @if(!empty($row->location->name))
                        ...
                        <a href="{{ route('event.search', ['location_id' => $row->location->id]) }}" class="text-sm text-blue-600 hover:underline flex items-center gap-1 font-medium">
                            {{$location->name ?? ''}}
                        </a>
                    @endif
                </div>
                ...
                <div class="flex flex-wrap gap-4 mt-4 text-gray-600 text-sm font-medium">
                    @if($start_time)
                        <div class="flex items-center gap-2">
                            <i class="fa fa-clock-o text-lg text-gray-400"></i> {{ __("Start Time: :time",['time'=>$start_time]) }}
                        </div>
                    @endif
                    @if($duration)
                        <div class="flex items-center gap-2">
                            <i class="fa fa-hourglass-half text-lg text-gray-400"></i> {{ $duration }}
                        </div>
                    @endif
                </div>
            </div>
