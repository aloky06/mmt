...
class Booking extends BaseModel
{
    ...
    {
        return $this->belongsTo(\Modules\Flight\Models\Partner::class, 'partner_id');
    }
    ...
    protected static function booted()
    {
        ...
        static::creating(function ($booking) {
            // Check for referral cookies and attach if valid
            if (request()->hasCookie('partner_id')) {
                $booking->partner_id = request()->cookie('partner_id');
                $booking->referral_session_token = request()->cookie('referral_token');
            }
        });

        static::updated(function ($booking) {
            // Handle Commission generation
            if ($booking->isDirty('status')) {
                if (in_array($booking->status, [self::PAID, self::COMPLETED, self::CONFIRMED])) {
                    if ($booking->partner_id && ! \Modules\Flight\Models\Commission::where('booking_id', $booking->id)->exists()) {
                        $partner = $booking->partner;
                        if ($partner && $partner->status == 'active') {
                            $commission_amount = 0;
                            if ($partner->commission_type == 'percentage') {
                                $commission_amount = ($booking->total * $partner->commission_value) / 100;
                            } else {
                                $commission_amount = $partner->commission_value;
                            }

                            \Modules\Flight\Models\Commission::create([
                                'partner_id' => $partner->id,
                                'booking_id' => $booking->id,
                                'booking_amount' => $booking->total,
                                'commission_amount' => $commission_amount,
                                'status' => 'pending' // Admins can manually mark as confirmed after stay is completed
                            ]);
                        }
                    }
                } elseif ($booking->status == self::CANCELLED) {
                    $commission = \Modules\Flight\Models\Commission::where('booking_id', $booking->id)->first();
                    if ($commission) {
