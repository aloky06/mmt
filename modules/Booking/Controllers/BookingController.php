...
//        event(new VendorLogPayment($booking));
        ...
        if(Auth::check()) {
            ...
            if(!$user){
                ...
                // User already exists, just log them in to link booking
                \Illuminate\Support\Facades\Auth::loginUsingId($user->id, true);
            }
            
            // Assign this booking to the new/existing user
        ...
        if($booking->pay_now > 0) {
            try {
                // Ensure session is saved before gateway redirects or calls send()
                session()->save();
