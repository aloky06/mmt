...
class Boat extends Bookable
{
    ...
    static public function getSeoMetaForPageList()
    {
        ...
        $meta['full_url'] = url(config('boat.boat_route_prefix'));
        return $meta;
    }
    ...
    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','create_user'])->where('object_id', $this->id)->where('object_model', 'boat')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('boat_review_number_per_page', 5));
    }
    ...
    public static function search(Request $request)
    {
        ...
        if (!empty($location_id = $request->query('location_id'))) {
            ...
            if ($country_location_ids !== false) {
                $model_boat->whereIn("bravo_boats.location_id", $country_location_ids);
            }
        }
        ...
        if (is_array($terms) and !empty($terms = array_filter($terms))) {
            $model_boat->join('bravo_boat_term as tt', 'tt.target_id', "bravo_boats.id")->whereIn('tt.term_id', $terms);
        }
        ...
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
            $distance_sql = "(6371 * acos(cos(radians(" . (float)$lat . ")) 
                            * cos(radians(bravo_boats.map_lat)) 
                            * cos(radians(bravo_boats.map_lng) - radians(" . (float)$lgn . ")) 
                            + sin(radians(" . (float)$lat . ")) 
                            * sin(radians(bravo_boats.map_lat))))";
            
            $model_boat->selectRaw("{$distance_sql} AS radius_distance");
            $model_boat->whereRaw("{$distance_sql} <= 400");
            
            if(empty($request->input("orderby"))){
                $model_boat->orderBy('radius_distance', 'ASC');
            }
        }
        ...
        switch ($orderby){
            case "price_low_high":
                ...
                break;
            case "price_high_low":
                $model_boat->orderBy("min_price", "desc");
                break;
            case "rate_high_low":
        ...
        return $model_boat->with(['location','hasWishList','translations'])->paginate($limit);
    }
