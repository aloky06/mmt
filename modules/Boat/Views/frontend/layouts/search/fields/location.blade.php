<div class="form-group custom-location-group" id="boat-location-wrapper" onclick="document.getElementById('boat-location-input').focus()">
    ...
    <div class="form-content" style="position:relative;">
        ...
        <input type="text" id="boat-location-input" placeholder="{{__("Where are you going?")}}" autocomplete="off" onkeyup="searchBoatLocation(this.value)" onclick="searchBoatLocation(this.value)" value="{{ request()->input('map_place') ?? $location_name }}">
        
        <!-- Hidden Inputs for Form Submission -->
        <input type="hidden" name="location_id" id="boat_location_id" value="{{request()->input('location_id')}}">
        <input type="hidden" name="map_place" id="boat_map_place" value="{{request()->input('map_place')}}">
        <input type="hidden" name="map_lat" id="boat_map_lat" value="{{request()->input('map_lat')}}">
        <input type="hidden" name="map_lgn" id="boat_map_lgn" value="{{request()->input('map_lgn')}}">

        <!-- Dropdown Container -->
        <div class="premium-autocomplete-dropdown" id="boat-location-dropdown"></div>
    </div>
</div>
...
#boat-location-input::placeholder {
    color: #ccc;
    font-weight: 600;
}
...
<script>
    ...
    function debounceBoat(func, wait) {
        let timeout;
    ...
    const searchBoatLocation = debounceBoat(async function(query) {
        const dropdown = document.getElementById('boat-location-dropdown');
        dropdown.innerHTML = '';
            ...
            if (data.features && data.features.length > 0) {
                data.features.forEach(feature => {
                    ...
                    item.onclick = function(e) {
                        ...
                        selectBoatLocation(name, geom[1], geom[0], subText);
                    };
    ...
    function selectBoatLocation(name, lat, lng, subtitle) {
        document.getElementById('boat-location-input').value = name;
        document.getElementById('boat_map_place').value = name;
        document.getElementById('boat_map_lat').value = lat;
        document.getElementById('boat_map_lgn').value = lng;
        document.getElementById('boat-location-dropdown').style.display = 'none';
    }

    // Hide dropdowns when clicking outside
    document.addEventListener('click', function(e) {
        let wrapper = document.getElementById('boat-location-wrapper');
        let dropdown = document.getElementById('boat-location-dropdown');
