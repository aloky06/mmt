@php
    ...
    // Review Data
    $reviewData = setting_item('boat_enable_review') ? $row->getScoreReview() : null;
@endphp
    ...
    <div class="flex-1 flex flex-col md:flex-row p-5 gap-6">
            ...
            <div>
                ...
                <div class="flex items-center gap-2 mb-3">
                    ...
                    @if(!empty($row->location->name))
                        ...
                        <a href="{{ route('boat.search', ['location_id' => $row->location->id]) }}" class="text-sm text-blue-600 hover:underline flex items-center gap-1 font-medium">
                            {{$location->name ?? ''}}
                        </a>
                    @endif
                </div>

                <div class="flex flex-wrap gap-4 mt-4 text-gray-600 text-sm font-medium">
                    @if($row->max_guest)
                        <div class="flex items-center gap-2" title="{{ __("Max Guests") }}">
                            <i class="icofont-ui-user-group text-lg text-gray-400"></i> {{ $row->max_guest }}
                        </div>
                    @endif
                    @if($row->cabin)
                        <div class="flex items-center gap-2" title="{{__("Cabin")}}">
                            <i class="icofont-sail-boat-alt-2 text-lg text-gray-400"></i> {{ $row->cabin }}
                        </div>
                    @endif
                    @if($row->length)
                        <div class="flex items-center gap-2" title="{{__("Length Boat")}}">
                            <i class="icofont-yacht text-lg text-gray-400"></i> {{ $row->length }}
                        </div>
                    @endif
                    @if($row->speed)
                        <div class="flex items-center gap-2" title="{{__("Speed")}}">
                            <i class="icofont-speed-meter text-lg text-gray-400"></i> {{ $row->speed }}
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="md:w-48 flex flex-col justify-between">
            ...
            <div class="text-right mt-auto">
                <div class="text-xs text-gray-500 font-medium uppercase">{{__("from")}}</div>
                <div class="text-2xl font-black text-gray-900 leading-none mt-1">
                    {{ format_money($row->min_price) }}
                </div>
