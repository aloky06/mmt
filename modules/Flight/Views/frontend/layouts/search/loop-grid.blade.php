@if(isset($isTravelpayouts) && $isTravelpayouts)
    <!-- Travelpayouts Live API Flight Card -->
    <div class="card w-100 shadow-hover-3 mb-4">
        <a href="#" class="d-block mb-0 mx-1 mt-1 p-3 text-center" tabindex="0">
            <img class="img-fluid" src="{{$row['airline_logo']}}" alt="{{$row['airline_name']}}" style="max-height: 50px;">
        </a>
        <div class="card-body px-3 pt-0 pb-3 my-0 mx-1">
            <div class="row">
                <div class="col-7">
                    <span class="card-title text-dark font-size-17 font-weight-bold">{{$row['airline_name']}}</span>
                    <div class="text-muted text-sm">{{$row['flight_number']}}</div>
                </div>
                <div class="col-5">
                    <div class="text-right">
                        <h6 class="font-weight-bold font-size-17 text-gray-3 mb-0">₹{{ number_format($row['price']) }}</h6>
                        <span class="font-weight-normal font-size-12 d-block text-color-1">{{__('total')}}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="mb-3">
            ...
            <div class="d-flex justify-content-center pl-3 pr-3">
                <a href="{{ $row['deep_link'] }}" target="_blank" class="btn btn-warning text-white font-weight-bold w-100">
                    {{__("Book on Travelpayouts")}} <i class="fa fa-external-link"></i>
                </a>
            </div>
        </div>
    </div>
@else
    <!-- Local Database Flight Card -->
    <div class="card w-100 shadow-hover-3 mb-4">
