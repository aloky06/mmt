...
class Car extends Bookable
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $package_type = $request->input('package_type', 'daily');
        $trip_type    = $request->input('trip_type', 'outstation_one_way');
        $distance_km  = (float) $request->input('distance_km', 0);

        if ($trip_type === 'hourly_rental') {
            if ($package_type === '4hr' && !empty($this->price_4hr)) {
                $total = $this->price_4hr * $number;
            } elseif ($package_type === '8hr' && !empty($this->price_8hr)) {
                $total = $this->price_8hr * $number;
            } else {
                $total = $this->price * $number;
            }
        } elseif (in_array($trip_type, ['outstation_one_way', 'outstation_round_trip', 'airport_transfer'])) {
            if (!empty($this->price_per_km) && $distance_km > 0) {
                $total = $this->price_per_km * $distance_km * $number;
            } else {
                $total = $this->price * $number;
            }
        } else {
        ...
        if ($check) {
            ...
            $booking->addMeta('package_type', $package_type);
            if($this->isDepositEnable())
            {
    ...
    {
        // Inject end_date if missing (One-Way, Airport Transfer, Hourly)
        ...
        if (strtotime($start_date) < strtotime('+1 hours')) {
            return $this->sendError(__("You must book at least 1 hour in advance"));
        }
    ...
    public function getBookingData()
    {
        ...
        $booking_data = [
            ...
            'buyer_fees'      => [],
            'start_date'      => request()->input('pickup_date') ?? request()->input('start') ?? "",
            'start_date_html' => $date_html ?? __('Please select'),
            'end_date'        => request()->input('return_date') ?? request()->input('end') ?? "",
            ...
            'price_4hr'=> $this->price_4hr,
            'price_8hr'=> $this->price_8hr,
            'price'=> $this->price,
            'trip_type' => request()->input('trip_type', 'outstation_one_way'),
            'distance_km' => request()->input('distance_km', 0),
            'hourly_package' => request()->input('hourly_package', '4hr')
        ];
        $lang = app()->getLocale();
    ...
    public static function search(Request $request)
    {
        ...
        if (!empty($location_id = $request->query('location_id'))) {
            ...
            // Geo-Location Filter
            $country_location_ids = get_country_filtered_location_ids();
            if ($country_location_ids !== false) {
                $model_car->where(function($q) use ($country_location_ids) {
                    $q->whereIn("bravo_cars.location_id", $country_location_ids)
                      ->orWhereNull("bravo_cars.location_id");
                });
            }
        }
        
        // ── Custom Radius Search (Max 400km) ─────────────────────────
        if (!empty($lat = $request->query('pickup_lat')) and !empty($lng = $request->query('pickup_lng'))) {
            $distance_sql = "(6371 * acos(cos(radians(" . (float)$lat . ")) 
                            * cos(radians(bravo_cars.map_lat)) 
                            * cos(radians(bravo_cars.map_lng) - radians(" . (float)$lng . ")) 
                            + sin(radians(" . (float)$lat . ")) 
                            * sin(radians(bravo_cars.map_lat))))";
            
            $model_car->selectRaw("{$distance_sql} AS radius_distance");
            $model_car->whereRaw("{$distance_sql} <= 400");
            
            // Order by nearest if no other order is specified
            if(empty($request->input("orderby"))){
                $model_car->orderBy('radius_distance', 'ASC');
            }
        }
        // ─────────────────────────────────────────────────────────────
