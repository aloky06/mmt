<div class="form-group custom-location-group" id="space-location-wrapper" onclick="document.getElementById('space-location-input').focus()">
    ...
    <div class="form-content" style="position:relative;">
        ...
        <input type="text" id="space-location-input" placeholder="{{__("Where are you going?")}}" autocomplete="off" onkeyup="searchSpaceLocation(this.value)" onclick="searchSpaceLocation(this.value)" value="{{ request()->input('map_place') ?? $location_name }}">
        
        <!-- Hidden Inputs for Form Submission -->
        <input type="hidden" name="location_id" id="space_location_id" value="{{request()->input('location_id')}}">
        <input type="hidden" name="map_place" id="space_map_place" value="{{request()->input('map_place')}}">
        <input type="hidden" name="map_lat" id="space_map_lat" value="{{request()->input('map_lat')}}">
        <input type="hidden" name="map_lgn" id="space_map_lgn" value="{{request()->input('map_lgn')}}">

        <!-- Dropdown Container -->
        <div class="premium-autocomplete-dropdown" id="space-location-dropdown"></div>
    </div>
</div>
...
#space-location-input::placeholder {
    color: #ccc;
    font-weight: 600;
}
...
<script>
    ...
    function debounceSpace(func, wait) {
        let timeout;
    ...
    const searchSpaceLocation = debounceSpace(async function(query) {
        const dropdown = document.getElementById('space-location-dropdown');
        dropdown.innerHTML = '';
            ...
            if (data.features && data.features.length > 0) {
                data.features.forEach(feature => {
                    ...
                    item.onclick = function(e) {
                        ...
                        selectSpaceLocation(name, geom[1], geom[0], subText);
                    };
    ...
    function selectSpaceLocation(name, lat, lng, subtitle) {
        document.getElementById('space-location-input').value = name;
        document.getElementById('space_map_place').value = name;
        document.getElementById('space_map_lat').value = lat;
        document.getElementById('space_map_lgn').value = lng;
        document.getElementById('space-location-dropdown').style.display = 'none';
    }

    // Hide dropdowns when clicking outside
    document.addEventListener('click', function(e) {
        let wrapper = document.getElementById('space-location-wrapper');
        let dropdown = document.getElementById('space-location-dropdown');
