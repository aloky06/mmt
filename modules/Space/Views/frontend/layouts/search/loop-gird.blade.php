@php
    ...
    // Review Data
    $reviewData = setting_item('space_enable_review') ? $row->getScoreReview() : null;
@endphp
            ...
            <div>
                ...
                <div class="flex items-center gap-2 mb-3">
                    ...
                    @if(!empty($row->location->name))
                        ...
                        <a href="{{ route('space.search', ['location_id' => $row->location->id]) }}" class="text-sm text-blue-600 hover:underline flex items-center gap-1 font-medium">
                            {{$location->name ?? ''}}
                        </a>
                    @endif
                </div>
                ...
                <div class="flex flex-wrap gap-4 mt-4 text-gray-600 text-sm font-medium">
                    @if($max_guests)
                        <div class="flex items-center gap-1.5 tooltip" title="{{ __("No. People") }}">
                            <i class="icofont-people text-lg text-gray-400"></i> {{$max_guests}}
                        </div>
                    @endif
                    @if($bed)
                        <div class="flex items-center gap-1.5 tooltip" title="{{ __("No. Bed") }}">
                            <i class="icofont-hotel text-lg text-gray-400"></i> {{$bed}}
                        </div>
                    @endif
                    @if($bathroom)
                        <div class="flex items-center gap-1.5 tooltip" title="{{ __("No. Bathroom") }}">
                            <i class="icofont-bathtub text-lg text-gray-400"></i> {{$bathroom}}
                        </div>
                    @endif
                    @if($square)
                        <div class="flex items-center gap-1.5 tooltip" title="{{ __("Square") }}">
                            <i class="icofont-ruler-compass-alt text-lg text-gray-400"></i> {!! size_unit_format($square) !!}
                        </div>
                    @endif
                </div>
            </div>
