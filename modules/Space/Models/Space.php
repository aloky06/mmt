    ...
    public static function search(Request $request)
    {
        ...
        if (!empty($location_id = $request->query('location_id'))) {
            ...
            if ($country_location_ids !== false) {
                $model_space->whereIn("bravo_spaces.location_id", $country_location_ids);
            }
        }
        ...
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
            $distance_sql = "(6371 * acos(cos(radians(" . (float)$lat . ")) 
                            * cos(radians(bravo_spaces.map_lat)) 
                            * cos(radians(bravo_spaces.map_lng) - radians(" . (float)$lgn . ")) 
                            + sin(radians(" . (float)$lat . ")) 
                            * sin(radians(bravo_spaces.map_lat))))";
            
            $model_space->selectRaw("{$distance_sql} AS radius_distance");
            $model_space->whereRaw("{$distance_sql} <= 400");
            
            if(empty($request->input("orderby"))){
                $model_space->orderBy('radius_distance', 'ASC');
            }
        }
