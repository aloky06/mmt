...
if (!empty($menus))
    ...
    // Hide non-Cab modules for Driver/Vendor Dashboard
    $allowed_vendor_menus = ['dashboard', 'car', 'booking-history', 'payout', 'profile', 'password', 'admin'];
    foreach ($menus as $k => $menuItem) {
        if (!in_array($k, $allowed_vendor_menus)) {
            unset($menus[$k]);
        }
    }
?>
<div class="sidebar-user">
    ...
    <div class="user-profile-plan">
        @if( !Auth::user()->hasPermissionTo("dashboard_vendor_access") and setting_item('vendor_enable'))
            @php
                $vendorRequest = \App\User::find(Auth::id())->vendorRequest;
            @endphp
            @if(!empty($vendorRequest) and $vendorRequest->status == 'pending')
                <span class="badge badge-warning" style="padding: 10px; font-size: 14px; background: #ffc107; color: #000; border-radius: 4px; display: block; width: 100%;">{{ __("Application Pending") }}</span>
            @else
