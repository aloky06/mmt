...
class UserController extends FrontendController
{
    ...
    public function sendOtp(Request $request)
    {
        ...
        if (!$user) {
            ...
            $user = \App\User::create([
                'email'      => $email,
                'first_name' => $request->input('first_name') ?? explode('@', $email)[0],
        ...
        if (in_array($user->status, ['blocked'])) {
            $errors = new MessageBag(['message_error' => __('Your account has been blocked')]);
            return response()->json(['error' => true, 'messages' => $errors], 200);
        }
        ...
        try {
            ...
            $errors = new MessageBag(['message_error' => __('Failed to send OTP email: ' . $e->getMessage())]);
            return response()->json(['error' => true, 'messages' => $errors], 200);
        }
    ...
    public function verifyOtp(Request $request)
    {
        ...
        if (!$user || $user->otp_code !== $request->input('otp_code') || now()->greaterThan($user->otp_expires_at)) {
            $errors = new MessageBag(['message_error' => __('Invalid or expired OTP')]);
            return response()->json(['error' => true, 'messages' => $errors], 200);
        }
