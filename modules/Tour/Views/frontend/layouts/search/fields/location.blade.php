<div class="form-group custom-location-group" id="tour-location-wrapper" onclick="document.getElementById('tour-location-input').focus()">
    ...
    <div class="form-content" style="position:relative;">
        ...
        <input type="text" id="tour-location-input" placeholder="{{__("Where are you going?")}}" autocomplete="off" onkeyup="searchTourLocation(this.value)" onclick="searchTourLocation(this.value)" value="{{ request()->input('map_place') ?? $location_name }}">
        
        <!-- Hidden Inputs for Form Submission -->
        <input type="hidden" name="location_id" id="tour_location_id" value="{{request()->input('location_id')}}">
        <input type="hidden" name="map_place" id="tour_map_place" value="{{request()->input('map_place')}}">
        <input type="hidden" name="map_lat" id="tour_map_lat" value="{{request()->input('map_lat')}}">
        <input type="hidden" name="map_lgn" id="tour_map_lgn" value="{{request()->input('map_lgn')}}">

        <!-- Dropdown Container -->
        <div class="premium-autocomplete-dropdown" id="tour-location-dropdown"></div>
    </div>
</div>
...
#tour-location-input::placeholder {
    color: #ccc;
    font-weight: 600;
}
...
<script>
    ...
    function debounceTour(func, wait) {
        let timeout;
    ...
    const searchTourLocation = debounceTour(async function(query) {
        const dropdown = document.getElementById('tour-location-dropdown');
        dropdown.innerHTML = '';
            ...
            if (data.features && data.features.length > 0) {
                data.features.forEach(feature => {
                    ...
                    item.onclick = function(e) {
                        ...
                        selectTourLocation(name, geom[1], geom[0], subText);
                    };
    ...
    function selectTourLocation(name, lat, lng, subtitle) {
        document.getElementById('tour-location-input').value = name;
        document.getElementById('tour_map_place').value = name;
        document.getElementById('tour_map_lat').value = lat;
        document.getElementById('tour_map_lgn').value = lng;
        document.getElementById('tour-location-dropdown').style.display = 'none';
    }

    // Hide dropdowns when clicking outside
    document.addEventListener('click', function(e) {
        let wrapper = document.getElementById('tour-location-wrapper');
        let dropdown = document.getElementById('tour-location-dropdown');
