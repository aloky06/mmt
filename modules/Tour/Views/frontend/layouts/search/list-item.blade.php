                ...
                <div class="flex flex-col">
                    ...
                    @else
                        <div class="bg-white rounded-xl border border-gray-200 p-12 text-center shadow-sm">
                            ...
                            <p class="text-gray-500">We couldn't find any tours matching your current filters. Try adjusting them or searching a different area.</p>
                            <button onclick="window.location.href='{{ route('tour.search') }}'" class="mt-6 bg-brand hover:bg-brand-dark text-white font-bold py-2.5 px-6 rounded-lg transition-colors">
                                Clear All Filters
                            </button>
                        </div>
                    @endif
                ...
                @if($rows->total() > 0)
                    <div class="mt-8 flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                        <div class="text-sm font-medium text-gray-500">
                            Showing <span class="font-bold text-gray-900">{{ $rows->firstItem() }}</span> to <span class="font-bold text-gray-900">{{ $rows->lastItem() }}</span> of <span class="font-bold text-gray-900">{{ $rows->total() }}</span> tours
                        </div>
