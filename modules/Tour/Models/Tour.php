    ...
    public static function search(Request $request)
    {
        ...
        if (!empty($location_id = $request->query('location_id'))) {
            ...
            if ($country_location_ids !== false) {
                $model_Tour->whereIn("bravo_tours.location_id", $country_location_ids);
            }
        }
        ...
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
            $distance_sql = "(6371 * acos(cos(radians(" . (float)$lat . ")) 
                            * cos(radians(bravo_tours.map_lat)) 
                            * cos(radians(bravo_tours.map_lng) - radians(" . (float)$lgn . ")) 
                            + sin(radians(" . (float)$lat . ")) 
                            * sin(radians(bravo_tours.map_lat))))";
            
            $model_Tour->selectRaw("{$distance_sql} AS radius_distance");
            $model_Tour->whereRaw("{$distance_sql} <= 400");
            
            if(empty($request->input("orderby"))){
                $model_Tour->orderBy('radius_distance', 'ASC');
            }
        }
