...
function get_user_country()
{
    ...
    if ($ip == '127.0.0.1' || $ip == '::1') {
        // Fallback for local testing to an Indian IP
        $ip = '103.116.170.0'; 
    }

    $country = 'IN'; // Default Fallback (Option B)
...
function get_country_filtered_location_ids() {
    $user_country = get_user_country();
    if (!$user_country) return false;
    
    $get_ids = function($country) {
        $all_location_ids = [];
        $locations = \Modules\Location\Models\Location::where('country', $country)->get();
        foreach($locations as $loc) {
            $all_location_ids[] = $loc->id;
            $descendants = \Modules\Location\Models\Location::descendantsOf($loc->id)->pluck('id')->toArray();
            $all_location_ids = array_merge($all_location_ids, $descendants);
        }
        return array_unique($all_location_ids);
    };

    $ids = $get_ids($user_country);
    
    if (empty($ids) && $user_country !== 'IN') {
        // Fallback to default country (IN) if no locations found for the detected country
        $ids = $get_ids('IN');
    }
